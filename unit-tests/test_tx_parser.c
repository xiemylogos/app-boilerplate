 ...
 uint8_t raw_tx[] = {
    ...
    */
    if(memcmp(tx.payload.contract_addr,ONG_ADDR,20) == 0 ) {
        assert_int_equal(sizeof(ONG_ADDR),21);
    } else {
...
static void test_personal_msg(void **state) {
    (void) state;

    uint8_t raw_personal_msg[] = {
        //personal msg length
        0x0D,
        //personal msg
        116,101,115,116,32,109,115,103,32,104,97,115,104
    };
    buffer_t buf = {.ptr = raw_personal_msg, .size = sizeof(raw_personal_msg), .offset = 0};
    assert_int_equal(buf.size,14);
    personal_msg_info info;
    parser_status_e status = personal_msg_deserialize(&buf, &info);
    assert_int_equal(status, PARSING_OK);
}
...
int main() {
	const struct CMUnitTest tests[] = {cmocka_unit_test(test_ont_tx_serialization),
		cmocka_unit_test(test_state_info_serialization),
                cmocka_unit_test(test_personal_msg),
                cmocka_unit_test(test_oep4_transaction),
                cmocka_unit_test(test_oep4_payload)
